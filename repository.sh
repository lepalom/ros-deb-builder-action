...
echo '```bash' > README.md
echo "echo \"deb [trusted=yes,arch=amd64] https://raw.githubusercontent.com/$GITHUB_REPOSITORY/$DEB_DISTRO-$ROS_DISTRO $DEB_DISTRO main" | sudo tee /etc/apt/sources.list.d/$REPOSITORY.list" >> README.md
...
echo '```' >> README.md
deb [trusted=yes] https://raw.githubusercontent.com/lepalom/ros-one-packages/bullseye-debian bullseye main